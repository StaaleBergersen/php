...
try {
    ...
    if (!isset($_SESSION['innlogging']) || $_SESSION['innlogging'] === false){
       // echo $_SESSION['innlogging'];
        ...
        echo "<br>Logg er lagret i databasen";
         // Logg mislykket innlogging til databasen
         $beskrivelse = "Koblet til databasen, men ikke logget inn. BRUKERNAVN:  " . $_POST['brukernavn'] . " Passord: " . $_POST['passord'];

    } else {
       // echo $_SESSION['innlogging'];
        //echo "<br>Du er logget inn";
        $beskrivelse = "Riktig pålogget med BRUKERNAVN:  " . $_POST['brukernavn'] . " Passord: " . $_POST['passord'];
    ...
    $beskrivelse = "Kunne ikke koble til databasen BRUKERNAVN:  " . $_POST['brukernavn'] . " Passord: " . $_POST['passord'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $_SESSION['brukernavn'] = $innsendtNavn;
        $sqlAdmin = "SELECT * FROM brukere WHERE brukernavn = '$innsendtNavn' AND rettighet = 'administrator'"; // SQL for å sjekke om brukeren har administratorrettigheter

        $stmtAdmin = $conn->prepare($sqlAdmin);// Gjør spørringen klar
        $stmtAdmin->execute();// Kjør spørringen
    
        $user = $stmtAdmin->fetch(PDO::FETCH_ASSOC);// Hent ut brukeren som en assosiativ array

        if ($user) {
            $_SESSION['administrator'] = true;
            echo "Brukeren har administratorrettigheter!";
        } else {
            $_SESSION['administrator'] = false;
            echo "Brukeren er IKKE administrator.";
        }

        header('Location: index.php');  // Send brukeren tilbake til startsiden
        $beskrivelse = "Innlogget med BRUKERNAVN:  " . $_POST['brukernavn'] . " Passord: " . $_POST['passord'];
    } else {
        // Hvis brukeren ikke finnes, vis en feilmelding
        echo "<br>Feil brukernavn eller passord!";
        $beskrivelse = " Feil brukernavn eller passord BRUKERNAVN:  " . $_POST['brukernavn'] . " Passord: " . $_POST['passord'];
    }
}


$loggSql = "INSERT INTO logg (brukernavn, beskrivelse) VALUES (:brukernavn, :beskrivelse)";
